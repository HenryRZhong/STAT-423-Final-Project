...
There were only 5 rows of our 6704 with N/A values, two of these being completely empty rows and the remaining three with N/A values in the salary and years of experience columns. These 5 rows were filtered out of our processed dataset.
...
Our next question was related to whether how strong of a predictor education level specifically is of salary, and whether the interaction between education level and years of experience is significant. This question has implications to the financial value of higher education relative to the salary return over a course of a career.

```{r message=F}
# Density plot of salary per education level

# first, order education levels
...
The density plots show a clear difference between individuals with only a high school education and all higher education levels. Specifically, the rows corresponding to a high school education appear to have a median income of less than 50,000 INR, which is only about \$603 USD, compared to higher median incomes for the Bachelor's, Master's, and PhD groups. There looks to be high variation in the salaries of individuals with Bachelor's, with a right skew and a median just above 50,000 INR. The distribution for individuals with Master's is centered higher but with higher variation, and is neither left nor right skewed. Finally, the PhD salary distribution has the highest median and a left skew.

These findings might imply that while higher education does have a clear correlation with higher salary, the variation in salary is much greater for higher education levels compared to high school. Fortunately there do not appear to be diminishing returns in education; that is, the difference in median salary between the PhD and Master's groups appears about the same as the difference between the High School and Bachelor's groups.

Our next step is to examine how salary changes with years of experience relative to education level. As our initial scatterplots showed a curved relationship between years of experience and salary, we have taken a square root transformation of years of experience which is shown below.
